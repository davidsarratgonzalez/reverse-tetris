...
/**
 * Find a legal path from spawn to the target placement using BFS,
 * then convert it into animation keyframes.
 *
 * Uses the same move set as the placement generator:
 * - Move left / right
 * - Soft drop (1 cell down)
 * - Rotate CW / CCW (with SRS wall kicks)
 *
 * BFS guarantees the shortest legal path.
 */
export function planAnimation(
  ...
  if (path.length === 0) {
    // No path found (shouldn't happen — placement was generated by the same BFS)
    // Fallback: just show at target
  ...
  // Convert every BFS step to a keyframe (1:1, no skipping)
